...
/* SELECT ROW_NUMBER() 
    OVER (
        ...
        CASE 
            WHEN end_date_time 
            IS NOT NULL AND end_date_time != '' THEN end_date_time 
            ELSE scheduled_date_time 
        END ASC, 
        id ASC
    ) 
    AS no,
...
ORDER BY 
  priority DESC,
  CASE
    WHEN end_date_time IS NOT NULL AND end_date_time != '' THEN end_date_time
    ELSE scheduled_date_time
  END ASC,
  id ASC; */
...
SELECT ROW_NUMBER() 
    OVER (
        ...
        CASE 
            WHEN end_date_time 
            IS NOT NULL AND end_date_time != 'NULL' THEN end_date_time 
            ELSE scheduled_date_time 
        END ASC, 
        id ASC
    ) 
